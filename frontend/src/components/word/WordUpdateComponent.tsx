...
const WordUpdateComponent = (props: WordUpdateComponentProps) => {
    ...
    const { data: allWords } = useQuery<Word[]>({
        queryKey: ["words"],
        queryFn: async () => {
            const { data } = await axios.get(`http://localhost:8080/api/words/`);
            return data;
        },
    });
    ...
    const { data: languages } = useQuery<Language[]>({
        queryKey: ["languages"],
        queryFn: async () => {
            const { data } = await axios.get(`http://localhost:8080/api/languages/`);
            return data;
        },
    });
    ...
    const updateMutation = useMutation({
        mutationFn: async (updatedWord: Word) => {
            await axios.put(`http://localhost:8080/api/words/${updatedWord.id}`, updatedWord);
        },
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ["words"] });
            queryClient.invalidateQueries({ queryKey: ["word", props.wordId] });
            showPopup("Zapisano słowo!");
        },
    });
    ...
    return (
        <>
            <FormContainer title={"Edit word"} onSubmit={handleSubmit}>
                ...
                <InputSelect title={"Language"} data={languages?.map((l) => [l.name, String(l.id)]) ?? []} value={String(word?.languageId || "")} onChange={handleWordLanguageSelectChange} />
            </FormContainer>
